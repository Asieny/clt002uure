...
public class ToolLocation {
	 ...
	 * @return
	 */
	public static boolean isGpsOPen(Context con) {
	 ...
	 * </per>
	 */
	public static void requestLocation(Context con,final InterfaceBDLocation callback,
		...
		LocationClientOption options = configOptions(con);
		options.setIsNeedAddress(true);
		mLocClient.setLocOption(options);
		mLocClient.start();
	}

	private static LocationClientOption configOptions(Context con) {
		// 定位相关参数
		...
		if (isGpsOPen(con)) {
			...
			if (ToolNetwork.getInstance().init(con)
					.isConnected()) {
